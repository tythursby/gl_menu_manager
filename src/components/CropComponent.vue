<template>
  <mdb-modal size='lg' v-show="show" @close="closeModal()" :removeBackdrop='false'>
    <mdb-modal-header>
      Crop Image - Product# {{productImageId}}
    </mdb-modal-header>
    <mdb-modal-body>
      <div>
    ...
    <mdb-modal-footer>
      ...
      <mdb-btn size="sm" style='float: right' color='default' @click.native="closeModal(productImageId)" darkWaves :disabled='loading'>cancel</mdb-btn>
  ...
  export default {
    ...
    methods: {
      ...
      async post() {
        ...
        this.axios
          .post('https://api.compassionate.cloud/photo/product', {
            org: 'greenleaf',
            product: this.productImageId,
            photo: base64result
          })
